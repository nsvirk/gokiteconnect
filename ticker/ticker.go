...
import (
	...
	"github.com/nsvirk/gokiteconnect/v4/models"
)
...
func New(userId string, enctoken string) *Ticker {
	ticker := &Ticker{
		userId:              userId,
		enctoken:            enctoken,
		url:                 tickerURL,
...
// SetEnctoken set access token.
func (t *Ticker) SetEnctoken(enctoken string) {
	t.enctoken = enctoken
}
...
func parsePacket(b []byte) (models.Tick, error) {
	...
	if len(b) == modeQuoteIndexPacketLength || len(b) == modeFullIndexLength {
		...
		if len(b) == modeFullIndexLength {
			...
			tick.Timestamp = models.Time{
				Time: time.Unix(int64(binary.BigEndian.Uint32(b[24:28])), 0),
			}
		}
